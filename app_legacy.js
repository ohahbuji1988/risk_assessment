...
window.app = {
  ...
  async callGroqAPI(itemName, baseMonth, currentMonth) {
    ...
    const prompt = `Analyze price change rate and 3 major cost drivers for procurement item '${itemName}' between ${baseMonth} and ${currentMonth}.
IMPORTANT:
1. 'rate' MUST be a realistic float number (e.g. 8.5 or 12.3 or -3.2).
2. 'drivers' MUST contain exactly 3 objects whose 'weight' sum up to 100%.
3. 'url' MUST be one of: "${refUrls.LME}", "${refUrls.BLS}", "${refUrls.OECD}", "${refUrls.EIA}". Choose LME for metals/ores/minerals, BLS for wages/labor, OECD for PPI/manufacturing, EIA for oil/gas/chemicals.
4. 'summary' and 'rationaleText' MUST be in natural Korean. Use <strong> HTML tags in rationaleText for key emphasis.

Return ONLY valid JSON in exact structure:
{
  "rate": 11.5,
  "drivers": [
    { "label": "원자재 모재", "weight": 45, "impact": 5.2, "url": "${refUrls.LME}", "sourceName": "LME Metal Index", "cloudDetail": "☁️ 원자재 급등 세부 설명", "baseVal": 100, "targetVal": 118.2 },
    { "label": "가공 인건비", "weight": 35, "impact": 4.1, "url": "${refUrls.BLS}", "sourceName": "US BLS ECI", "cloudDetail": "☁️ 노무비 상승 세부 설명", "baseVal": 100, "targetVal": 108.5 },
    { "label": "PPI 물가지수", "weight": 20, "impact": 2.2, "url": "${refUrls.OECD}", "sourceName": "OECD PPI", "cloudDetail": "☁️ 생산자물가 세부 설명", "baseVal": 100, "targetVal": 104.1 }
  ],
  "summary": "한국어 가격 변동 요약 설명 1~2문장",
  "rationaleTitle": "📌 '${itemName}' 원가 구조 비중 산출 근거",
  "rationaleText": "한국어로 3가지 항목 비중 산출 사유를 HTML strong 태그를 써서 가독성 좋게 설명"
}`;
    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: modelToUse,
        messages: [
          { role: 'system', content: 'You are an expert procurement cost analyst. Always respond in pure valid JSON.' },
...
Return ONLY valid JSON in exact structure:
{
  ...
  async triggerAIAnalysis() {
    ...
    // 1. Instant built-in fallback sync
    state.currentAnalysis = builtInEngine.analyze(itemName, baseMonth, currentMonth);
    this.renderAnalysisUI(itemName, false);

    // 2. Fetch live AI analysis if API key present
    let isLiveSuccess = false;
    if (state.apiKey) {
      try {
        const aiResult = await this.analyzeWithAI(itemName, baseMonth, currentMonth);
        if (aiResult && aiResult.drivers && aiResult.drivers.length > 0) {
  ...
  renderAnalysisUI(itemName, isComplete = true) {
    ...
    if (rateInput) rateInput.value = state.currentAnalysis.rate;
  ...
  updateTrendLineChart() {
    ...
    state.charts.trend.data.datasets = state.currentAnalysis.drivers.map((d, index) => {
      const start = d.baseVal || 100;
      const end = d.targetVal || 110;
      ...
      const data = labels.map((lbl, idx) => {
        ...
        const val = start + (end - start) * progress + (Math.sin(idx * 1.5) * 0.8);
